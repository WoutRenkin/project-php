...
@section('main')
        ...
        <div class="card-body">
            ...
            <div class="row mt-4">
                <div class="col-sm-4 mb-4">
                    <button class="btn btn-primary" id="moreNotifications" data-toggle="tooltip"
                            title="Laad extra meldingen">Meer meldingen <i class="fas fa-sync-alt"></i></button>
                </div>
            </div>
        </div>
        <hr>
    ...
    <script>
        ...
        let notificationCount = $('#sessionNav').data("count");
        ...
        function postMarkRead(id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#sessionNav').attr('data-count', notificationCount)
                    $('#mainNavNotifications').attr('data-count', notificationMainNav)
                    $('#navDropSession').html(notificationCount)
                    $('#sessionMessage').show();
                    timeOutFunction()
                },
                error: function (e) {
        ...
        function loadNotifications() {
            $.ajax({
                type: 'GET',
                url: `session/getReadNotifications?page=${page}`,
                data: {
                            ...
                            if ($("#NotificationsFilter").val() == 1) {
                                html = `
                                        <div class="alert alert-success shadow" role="alert" name="">
                                            [${date.toLocaleString('fr-BE')}] Student ${value.data["user"]} van <b>team ${value.data["team"]}</b> heeft een sessie aangevraagd:
                                            <a class="btn btn-primary markAsRead" href="/admin/voorstel/${value.data.session["id"]}/edit" data-id="${value["id"]}">
                                                Bekijk sessie
                                            </a>
                                ...
                                html = `
                                        <div class="alert alert-info shadow" role="alert" name="">
                                            [${date.toLocaleString('fr-BE')}] Student ${value.data["user"]} van <b>team ${value.data["team"]}</b> heeft een sessie aangevraagd:
                                            <a class="btn btn-primary" href="/admin/voorstel/${value.data.session["id"]}/edit">
                                                Bekijk sessie
                                            </a>
                                         </div>`
                            }
